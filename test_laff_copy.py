import unittest
import numpy as np
from LAFF.laff_copy import laff_copy
...
class LaffCopyTest(unittest.TestCase):
    ...
    def test_column_column_copy(self):
        np.testing.assert_allclose(laff_copy(self.x, self.y), self.x)

    def test_column_row_copy(self):
        np.testing.assert_allclose(laff_copy(self.x, self.y.T), self.x.T)

    def test_row_column_copy(self):
        np.testing.assert_allclose(laff_copy(self.x.T, self.y), self.x)

    def test_row_row_copy(self):
        np.testing.assert_allclose(laff_copy(self.x.T, self.y.T), self.x.T)

    def test_column_column_worong_size(self):
        self.assertRaises(Exception, laff_copy, self.x, self.z)

    def test_column_row_worong_size(self):
        self.assertRaises(Exception, laff_copy, self.x, self.z.T)

    def test_row_column_worong_size(self):
        self.assertRaises(Exception, laff_copy, self.x.T, self.z)

    def test_row_row_worong_size(self):
        self.assertRaises(Exception, laff_copy, self.x.T, self.z.T)
